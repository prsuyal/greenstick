...
const DashboardPage = ({ user: initialUser, onLogout }) => {
  ...
  const fetchLatestUserData = async () => {
    try {
      const response = await fetch(
        `https://api.greenstickusa.com/api/users/${initialUser.id}`
      );
      if (response.ok) {
  ...
  const DashboardCard = ({
    ...
    return (
      <div className={`flex flex-col ${isLocked ? "cursor-not-allowed" : ""}`}>
        <h2 className="text-2xl 2xl:text-3xl font-medium mb-3 2xl:mb-4">
          {title}
        </h2>
        <div
  ...
  const LearningCard = () => {
    ...
    const handleLessonClick = (lesson) => {
      navigate(
        `/level-${lesson.level}/${lesson.sublevel.toLowerCase()}/lesson-${
          lesson.number
        }`
      );
    };
  ...
  const DictionaryCard = () => (
    <div className="relative h-full">
      <p className="font-medium text-lg 2xl:text-2xl mb-2 2xl:mb-4">
        Dictionary
      </p>
      <img
        src={DictionaryImage}
        alt="Dictionary"
        className="absolute bottom-2 2xl:bottom-4 right-2 2xl:right-4 h-40 2xl:h-48 object-contain"
      />
    </div>
  );
  ...
  const ExoCard = () => (
    <Link to="/exo">
      <div className="relative h-full">
        <p className="font-medium text-lg 2xl:text-2xl mb-2 2xl:mb-4">
          Meet <span className="text-gs-dark-green">Exo</span>
        </p>
        <img
          src={ExoLogo}
          alt="Exo logo"
          className="absolute pt-6 2xl:pt-8 h-20 2xl:h-24 object-contain"
        />
        <img
          src={ExoImage}
          alt="Exo Chat"
  ...
  const MobileView = () => (
    <div className="h-screen p-4 flex flex-col font-[Poppins]">
      <div className="flex justify-between items-center w-full mb-10">
        <img src={gsLogoBlack} alt="Greenstick logo" className="h-12" />
      ...
      <div className="text-center">
        ...
        <p className="text-lg">
          We apologize, but our app is not currently supported on mobile
          devices.
        </p>
      ...
      <div className="hidden lg:flex h-screen bg-white overflow-hidden">
        ...
        <div className="w-64 2xl:w-72 bg-[#edf6f0] p-6 2xl:p-8 flex flex-col">
          ...
          <div className="mb-2 2xl:mb-4">
            <p className="text-center font-semibold mb-2 text-base 2xl:text-xl">
              {currentUser?.plan || "No Plan"}
            </p>
          ...
          <div className="mx-auto text-center">
            <button
              ...
              onClick={() =>
                (window.location.href = "mailto:support@greenstickusa.com")
              }
            >
        ...
        <div className="flex-grow pt-10 2xl:pt-12 p-10 2xl:p-12 flex flex-col overflow-y-auto">
          <div className="flex justify-between items-center mb-6 2xl:mb-10">
            ...
            <div className="relative" ref={settingsButtonRef}>
              ...
              {settingsOpen && (
                <div
                  className="absolute right-0 mt-2 w-56 2xl:w-64 bg-white border rounded-md z-50 "
                  ref={settingsMenuRef}
                >
                  <div>
                    <button
                    ...
                    >
                      <FaMinus className="mr-3 2xl:mr-4 text-black" />
                      <span>Delete Account</span>
                    </button>
                    <button
                      onClick={handleManageSubscription}
